...
## Overview
This standalone skill enables any agent to search for words or phrases inside the book of Joshua in local SQLite Bible databases stored in `/workspace/biblemate/data/bibles` or `/workspace/biblemate/data_custom/bibles`.

## Guidelines & Objectives
When executing this skill:
- Always run the python retriever script located at `/home/box/agent-data/biblemate-native-skills/search/search_retriever.py` with the `--book Josh` option to perform the search.
- Execute the script using: `python3 /home/box/agent-data/biblemate-native-skills/search/search_retriever.py --book Josh "<query>"` where `<query>` is the input prompt or arguments.
...
Folded from the Grok Build command `/Josh` — treat “user arguments after the slash command” as the input the user provided after the slash.
