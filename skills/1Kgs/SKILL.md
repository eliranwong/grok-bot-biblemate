...
## Overview
This standalone skill enables any agent to search for words or phrases inside the book of 1 Kings in local SQLite Bible databases stored in `/workspace/biblemate/data/bibles` or `/workspace/biblemate/data_custom/bibles`.

## Guidelines & Objectives
When executing this skill:
- Always run the python retriever script located at `/home/box/agent-data/biblemate-native-skills/search/search_retriever.py` with the `--book 1Kgs` option to perform the search.
- Execute the script using: `python3 /home/box/agent-data/biblemate-native-skills/search/search_retriever.py --book 1Kgs "<query>"` where `<query>` is the input prompt or arguments.
...
Folded from the Grok Build command `/1Kgs` — treat “user arguments after the slash command” as the input the user provided after the slash.
