...
- Always run the python converter script located at `/home/box/agent-data/biblemate-native-skills/md/md_converter.py` to perform the conversion or export.
- Execute the script using: `python3 /home/box/agent-data/biblemate-native-skills/md/md_converter.py "<query>"` where `<query>` is the input prompt or arguments containing the file path or export directives (e.g. `whole`, `conversation`).
...
Folded from the Grok Build command `/md` — treat “user arguments after the slash command” as the input the user provided after the slash.
