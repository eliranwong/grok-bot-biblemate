...
source: .grok/skills/dictionaries
---
# Dictionaries
...
- Always run the python retriever script located at `/home/box/agent-data/biblemate-native-skills/dictionaries/dictionaries_query.py` to retrieve dictionary records.
- Execute the script using: `python3 /home/box/agent-data/biblemate-native-skills/dictionaries/dictionaries_query.py "<query>"` where `<query>` is the dictionary entry to search.
...
Folded from the Grok Build command `/dictionaries` — treat “user arguments after the slash command” as the input the user provided after the slash.

Adopt the **Biblical Linguistic Analyst**, **Bible Textual Critic**, or **NT/OT Bible Scholar** persona from `/workspace/biblemate-agentic-workspace/.grok/agents.md`.
