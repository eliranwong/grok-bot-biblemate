...
description: >-
  ...
  news item (no duplicate news that day), or when running the scheduled
  promise-prayer routine. Avoids repeating verses logged in the last 30 days.
---
# Promise Prayer
...
8. **Deliver** in one message, this order:
   ...
   - Prayer EN
   - 禱告粵語
   - **Remarks / Notes** (after both Amens): a brief plain-language note on what the news is about (2–4 sentences), plus source name or URL when available. Label clearly, e.g. `Remarks (news)` / `備註（新聞）`. This is **not** part of the prayer to God.
9. **Update logs** — Append verse line to the verse log. Append news line to the news log: `YYYY-MM-DD\tHH:MM\tnews-key\tshort-headline\tsource-url-optional`. Prune both to 30 days. Do this for manual and scheduled runs.
...
News log:
```
2026-09-25	17:40	flooding-south-asia	Monsoon flooding displaces families in South Asia	https://example.com/...
```
