...
## Overview
This standalone skill enables any agent to dynamically check and list available versions of Bibles, commentaries, and lexicons stored in the standard (`/workspace/biblemate/data/...`) and custom (`/workspace/biblemate/data_custom/...`) directories.

## Guidelines & Objectives
When executing this skill:
- Always run the python lister script located at `/home/box/agent-data/biblemate-native-skills/data/data_lister.py`.
- Execute the script using: `python3 /home/box/agent-data/biblemate-native-skills/data/data_lister.py "<resource_type>"` where `<resource_type>` is `bible`, `commentary`, or `lexicon`.
...
Folded from the Grok Build command `/data` — treat “user arguments after the slash command” as the input the user provided after the slash.
